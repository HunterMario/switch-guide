...
Any console purchased after approximately August 2018 is **likely** to be patched.
...
Several methods and designs to do this exist, see [our guide](user_guide/rcm/entering_rcm) for more information.
...
These jigs are made in a way that they slide into the right joycon rail and have a piece of connected wire that then bridges pin 10 and one of the ground pins on the Switch.
A lot of different designs for these jigs exist, but it is important to understand, that these jigs can damage the Switch if they are made poorly.
...
Premade jigs can be found on online marketplaces for low cost.
...
If your Switch is one of the new hardware revisions that patched the exploit in RCM and you are on firmware 4.1.0 or lower, you should not update if you want to use CFW.
...
- **"Old"** *Patched Switch (HAC-001): Do NOT update past 4.1.0 if you do not want a modchip. Units on 4.1.0 can run CFW via the "caffeine" vulnerability, which is not covered on this guide.*

- **"New"** *Switch, Switch Lite and Switch OLED (HAC-001(-01), (HDH-001), (HEG-001): All of these consoles are "Mariko" consoles, and do not ship on vulnerable firmwares. CFW can still be ran on these consoles when modchipped.*
...
Homebrew can come in two different formats, namely in `nro`, `ovl` and `bin` files.

- `nro` Files are placed in the `switch` folder on your microSD card and can be launched using the Homebrew menu.
- `bin` This format is used as a payload and is to be pushed in RCM using a payload launcher like [tegrarcmgui](https://github.com/eliboa/TegraRcmGUI) on windows and [JTegraNX](https://github.com/dylwedma11748/JTegraNX) on other operating systems.
- `ovl` This format is used for overlays, meant to be used by [Tesla-Menu](https://github.com/WerWolv/Tesla-Menu), which is loaded by [nx-ovlloader](https://github.com/WerWolv/nx-ovlloader).
**Homebrew risks**
...
Make sure you are using a high quality microSD card from reputable brands, such as Samsung, Kingston, SanDisk, etc.

The recommended filesystem format is FAT32. While the Switch supports exFAT through an additional update from Nintendo, the filesystem driver used by Horizon is prone to corrupting and as a result is not advisable.
